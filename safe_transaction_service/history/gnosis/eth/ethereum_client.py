...
class ParityManager:
    ...
    def trace_blocks(self, block_identifiers: List[BlockIdentifier]) -> List[List[Dict[str, Any]]]:
        if not block_identifiers:
            return []
        payload = [{'id': i, 'jsonrpc': '2.0', 'method': 'Trace.Block',
        ...
        response = self.ethereum_client.http_session.post(self.ethereum_node_url2, json=payload)
        if not response.ok:
            message = f'Problem calling batch `Trace.Block` on blocks={block_identifiers} ' \
    ...
    def trace_transactions(self, tx_hashes: Sequence[EthereumHash]) -> List[List[Dict[str, Any]]]:
        ...
        if not tx_hashes:
            return []
        payload = [{'id': i, 'jsonrpc': '2.0', 'method': 'debug_traceTransaction',
...
class EthereumClient:
    ...
    def get_transaction_receipt(self, tx_hash: EthereumHash, timeout=None) -> Optional[TxReceipt]:
        try:
            ...
            # Parity returns tx_receipt even is tx i s still pending, so we check `blockNumber` is not None
